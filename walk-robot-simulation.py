class Solution(object):
    def robotSim(self, commands, obstacles):
        ...
        :rtype: int
        """
        leftface = {'N':'W', 'E':'N', 'S':'E', 'W':'S'}
        rightface = {'N':'E', 'E':'S', 'S':'W', 'W':'N'}
        dr = {'N':1,'S':-1,'E':1,'W':-1}
        obs = set()
        [obs.add(tuple(v)) for v in obstacles]
        cf,cx,cy = 'N', 0,0
        res = -1 
        for cmd in commands:
            print cf, (cx, cy), 'cmd ', cmd
            if cmd == -2: cf = leftface[cf]
            elif cmd == -1: cf = rightface[cf]
            else:
                st,xchg = (cx,True) if (cf=='E' or cf=='W') else (cy,False)
                inc, obsf = dr[cf], False
                en = st + inc*cmd
                print '  ',xchg, st, en, inc 
                for pos in xrange(st,en,inc):
                    if xchg and (pos+inc,cy) in obs: 
                        obsf = True
                        break
                    elif (not xchg) and (cx,pos+inc) in obs: 
                        obsf = True
                        break  
                    #else: pass
                if obsf:
                    print '    obstruct found ', cf, '@',pos+inc
                    if xchg: cx = pos 
                    else: cy = pos
                else:
                    if xchg: cx += dr[cf]*cmd
                    else: cy += dr[cf]*cmd
