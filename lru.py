class LRUCache(object):
    ...
    def __init__(self, capacity):
        self.h, self.cnt, self.capacity = {}, 0, capacity
        self.head, self.tail = self.Node('h', 'h'), self.Node('t', 't')
        self.head.next = self.tail
        self.tail.prev = self.head

    def update(self,key):
        return self.insert(self.unlink(self.h[key]))

    def get(self, key):
        #self.ec()
        if not self.h.has_key(key): return -1
        n = self.update(key)
        return n.value
    ...
    def put(self, key, value):
        ...
        if self.cnt < self.capacity:
            if self.h.has_key(key):
                ...
                self.h[key] = self.Node(key,value)
                self.insert(self.h[key])
                self.cnt += 1
            return
